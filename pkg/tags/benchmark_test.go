...
var funcNameToTestFunc = map[funcName]testFunc{
	...
	stagsGetByName: func(i int) (fillExisting func(), testBody func(tag int, name string)) {
		...
		testMap := tags.New[string](r1)
		return func() {
				fillTagsMap(i, testMap)
			},
			func(tag int, name string) {
...
func fillTagsMap(nIterates int, testMap *tags.TagMap[string]) {
	for n := 0; n <= nIterates; n++ {
...
func fillMap(nIterates int, testMap map[string]string) {
	for n := 0; n <= nIterates; n++ {
		strVal := strconv.Itoa(n)
		testMap[strVal] = strVal
	}
}
